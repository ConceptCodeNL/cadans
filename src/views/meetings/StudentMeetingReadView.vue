    ...
    <div v-else>
      <div class="mb-8">
        ...
        <div class="flex items-center gap-4 mt-4 flex-wrap">
          ...
          <span class="inline-block px-3 py-1 rounded-full text-sm font-semibold bg-primary-light text-primary-dark border border-primary">
            {{ $t('sessions.studentRow') }}
          </span>
      ...
      <!-- No data yet -->
      <div v-if="!hasStudentData" class="text-center py-16 bg-surface rounded-xl border border-border">
        ...
        <p class="text-text-secondary text-lg">{{ $t('meetings.studentNoDataYet') }}</p>
      </div>
      ...
      <div v-else class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            ...
            <p class="px-4 py-3 border border-border rounded-lg bg-surface-elevated text-text-primary">
              {{ meeting.student_meeting_date ? formatDate(meeting.student_meeting_date) : $t('common.notSet') }}
            </p>
          </div>
            ...
            <div class="grid grid-cols-3 gap-2">
              <div
                :class="[
                  ...
                  meeting.student_overall_grade === 'bad'
                    ? 'bg-error text-white'
              ...
              <div
                :class="[
                  'px-3 py-3 rounded-xl font-semibold text-sm text-center',
                  meeting.student_overall_grade === 'go_but_needs_attention'
              ...
              <div
                :class="[
                  'px-3 py-3 rounded-xl font-semibold text-sm text-center',
                  meeting.student_overall_grade === 'all_good'
                    ...
                    <div class="grid grid-cols-5 gap-2">
                      <div
                        ...
                        :class="[
                          ...
                          studentScores[comp.id] === level.value
                            ? level.activeClass
                            : 'bg-surface border-2 border-border opacity-40'
                        ]"
                      >
        ...
        <!-- General Notes -->
        <div v-if="meeting.student_general_notes">
...
const studentScores = computed(() => meeting.value?.student_competency_scores || {})
const studentNotes = computed(() => meeting.value?.student_competency_notes || {})

const hasStudentData = computed(() => {
  if (!meeting.value) return false
  const scores = meeting.value.student_competency_scores || {}
...
const averageAdvice = computed(() => {
  ...
  comps.forEach(comp => {
    const score = studentScores.value[comp.id]
    if (score != null) {
      hasAny = true
...
function getCompetencyTipsTops(compId) {
  const notes = studentNotes.value[compId]
